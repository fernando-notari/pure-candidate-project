...
function FilterTab({
    ...
    const textStyle = useAnimatedStyle(() => ({
        color: interpolateColor(progress.value, [0, 1], ["#555558", color]),
    }));
...
export function FilterTabs<T extends string>({
    ...
    const handlePress = useCallback(
        (filter: T | null, index: number) => {
            ...
            if (filter === null) {
                lineColor.value = withTiming(defaultAccent.color, {
                    duration: 150,
                });
            } else {
                const accent = accentColors?.[filter];
                lineColor.value = withTiming(
                    accent?.color ?? defaultAccent.color,
                    { duration: 150 },
                );
            }
        },
        ...
        <View style={styles.container}>
            <View style={styles.row}>
                {allFilters.map((filter, index) => {
                    const accent =
                        filter !== null ? accentColors?.[filter] : undefined;
