...
import { type ActionChipData, ActionChipsTicker } from "./action-chips-ticker";
                ...
                <View style={styles.content}>
                    <Text style={styles.stakes}>
                        {stakes.split("/").map((part, i, arr) => {
                            const key = `${i}-${part.trim()}`;
                            return (
                                <Text key={key}>
                                    {part}
                                    {i < arr.length - 1 && (
                                        <Text style={styles.stakesSlash}>
                                            {" "}
                                            /{" "}
                                        </Text>
                                    )}
                    ...
                    <View style={styles.statsRow}>
                        <Text style={styles.statText}>
                            <Text style={styles.statNumber}>{seatsOpen}</Text>{" "}
                            {seatLabel} open
                        </Text>
