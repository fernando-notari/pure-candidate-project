...
function FloatingChip({
    ...
    const config = FLOAT_CONFIGS[
        index % FLOAT_CONFIGS.length
    ] as (typeof FLOAT_CONFIGS)[number];
    ...
    return (
        <View style={styles.container} key={renderKey}>
            {visible.map((action, index) => {
                const p = PLACEMENTS[index] as (typeof PLACEMENTS)[number];
                const reverseIndex = visible.length - 1 - index;
                return (
                    <Animated.View
                        key={action.id}
                        style={[styles.chip, { top: p.top, left: p.left }]}
                        entering={FadeIn.delay(
                            reverseIndex * BASE_DELAY,
                        ).duration(400)}
                    >
